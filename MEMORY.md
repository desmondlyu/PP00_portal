...
### 專案定位
本專案（PP00 Tool Portal）是半導體記憶體研發與量化科技的工具入口網站，以優雅的 Bento 網格風格呈現，用以集中調用與展示九大核心工具（TTO 分析、JB Lab 借機系統、FT 特性分析、CP DL 分析、CP Datalog-to-Excel 轉換器、CP/FT Yield Auto Summary，以及待開發的 CP MSS 轉換工具、Dongle Auto Summary、WRITER）。
...
- **問題**：`logo.png` 在 GitHub Pages 顯示為 404。
  * **解法**：在 `.gitignore` 取消對根目錄 `logo.png` 的忽略，確保打包複製的 logo 檔案有成功推送到 GitHub 根目錄。
...
- [ ] 管理員驗證改為 server-side（目前為前端 hash 比對，可被 localStorage 直接繞過，僅為 soft toggle）。
