(function (root, factory) {
  ...
  function applySpecEdit({
    ...
    const specByRowIdx = new Map(
      specs.map((spec) => [String(spec.rowIdx), spec])
    );
    const targetItem = String(target.item ?? "");
    const targetDescription = String(target.description ?? "");
    const linkedIds = new Set(
      Array.from(
        ...
        .map(String)
        .filter((rowIdx) => {
          const linkedSpec = specByRowIdx.get(rowIdx);
          return (
            linkedSpec &&
            String(linkedSpec.item ?? "") === targetItem &&
            String(linkedSpec.description ?? "") === targetDescription
          );
        })
    );
