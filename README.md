...
*   **核心功能與特色**：
    *   **100% 本地安全防護**：基於純前端技術（ExcelJS），無需任何後端 API，所有測試規格書等機密資料絕不上傳，保障企業安全。
    ...
    *   **自訂 SPEC 注入**：支援啟用 CP_Mapping 規格對照表自訂 SPEC 注入，並可切換「僅填補空白」或「覆蓋所有」模式。
    *   **高質感標題與字型樣式**：匯出的 Excel 每個站點標題列自動套用「橘底黑字加粗」樣式，且所有儲存格預設字型調整為 Arial 10。
